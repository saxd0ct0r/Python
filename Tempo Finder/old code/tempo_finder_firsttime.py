...
while True:     # Main loop, keep running with user-selected starting tempo
    ...
    while step_size_index < len(STEP_SIZES):    # Limits number of repetitions
        ...
        success = None
        user_input = input(f"Attempt at {tempo}bpm. Success? (y/n): ")
        ...
        while success == None:
            if user_input.lower() == "y":
                ...
                user_input = input("Please answer (y/n): ")

        tempo_increases = True if success else False
        # print(f"{tempo}", end=" ")
