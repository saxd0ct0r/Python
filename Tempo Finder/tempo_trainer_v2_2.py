...
def step_to_bpm_quantized(step, target_bpm):
    """Convert step to nearest preset tempo from TEMPOS list"""
    calculated = step_to_bpm(step, target_bpm)
    # Find nearest tempo in TEMPOS
    closest_tempo = min(TEMPOS, key=lambda x: abs(x - calculated))
    return closest_tempo

def step_to_percent(step):
    """Convert step to percentage change from target"""
    multiplier = step_to_multiplier(step)
    return round((multiplier - 1) * 100, 1)

def print_tempo_info(current_step, target_bpm, current_measure):
    """Print both calculated and quantized tempo information"""
    tempo_calc = step_to_bpm(current_step, target_bpm)
    tempo_quant = step_to_bpm_quantized(current_step, target_bpm)
    percent = step_to_percent(current_step)
    
    print(f"Playing from measure {current_measure} at step {current_step:+d}")
    print(f"  Calculated: {tempo_calc} BPM ({percent:+.1f}%)")
    print(f"  Quantized:  {tempo_quant} BPM")
...
def find_tempo_targeted(target_bpm, start_measure, end_measure):
    ...
    print(f"Playing from measure {current_measure} at step {current_step:+d}")
    print(f"  Calculated: {tempo_calc} BPM ({percent:+.1f}%)")
    print(f"  Quantized:  {tempo_quant} BPM (nearest preset)")
    ...
    if measure_input.strip() == "":
        ...
        print(f"✓ Completed entire passage")
        print(f"  Calculated: {tempo_calc} BPM")
        print(f"  Quantized:  {tempo_quant} BPM")
        return {
            'hot_spot_measure': None,
    ...
    if measure_input.strip() != "":
        ...
        for drop_amount in [8, 4, 2, 1]:
            ...
            print_tempo_info(current_step, target_bpm, current_measure)
            measure_input = input(f"  Enter measure where error occurred (or [Enter] if completed cleanly): ")
    ...
    print(f"  Calculated: {step_to_bpm(hot_spot_step, target_bpm)} BPM ({step_to_percent(hot_spot_step):+.1f}%)")
    print(f"  Quantized:  {step_to_bpm_quantized(hot_spot_step, target_bpm)} BPM")
    print(f"{'='*60}\n")
