...
def generate_filtered_modes_chart(start_column=9, min_pcs=3):
    ...
    # Generate Markdown table
    header = "| Parent Collection | Mode                 | " + " | ".join(f"{i:<3}" for i in range(13)) + " | Secondary Mode       | Weighting Function |\n"
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"- Pitch classes ({start_column}–12) show the solfege syllable if present in the mode, relative to the mode's root as pitch class 0; empty if not present. PC 12 doubles the octave.\n")
...
def generate_stacked_tables(min_pcs=3):
    """
    Generate a single Markdown table combining filtered modes for start_column from 9 to 0,
    ...
    priority. Includes a single notes section at the end.
    ...
    all_rows = []
    for start_column in range(9, -1, -1):  # From 9 to 0 for narrowest first
        for collection, data in collections.items():
            for mode_idx, mode in enumerate(data['modes']):
                # Get pitch classes and solfege for this mode
                pcs = data['intervals'][mode_idx]
                ...
                if pc_count >= min_pcs:
                    ...
                    # Store row with weight, collection, secondary mode, and row for priority sorting
                    row = f"| {collection:<17} | {mode:<20} | {' | '.join(f'{x:<3}' for x in pc_row)} | {secondary_mode:<20} | {weight} |"
                    all_rows.append((int(weight), collection, secondary_mode, row))

    # Group rows by weighting function and filter duplicates
    grouped_rows = defaultdict(list)
    for weight, collection, secondary_mode, row in all_rows:
        grouped_rows[weight].append((collection, secondary_mode, row))
    ...
    # Write to Markdown file
    with open('Stacked_Filtered_Modes_Pitch_Classes_Chart.md', 'w', encoding='utf-8') as f:
        f.write("# Stacked Filtered Modes Pitch Classes Chart\n\n")
        f.write(header)
        if filtered_rows:
            f.write('\n'.join(row[1] for row in filtered_rows))
        else:
            f.write("No modes meet the filtering criteria.\n")
        f.write("\n\n**Notes:**\n")
        f.write(f"- Combines filtered modes for start_column from 9 to 0, with at least {min_pcs} pitch classes in the range [start_column, 12]. Non-included columns (0 to start_column-1) are obscured with 'xxx'.\n")
        f.write(f"- For duplicate weighting function values, retains the mode with the highest priority based on collection priority (Major > H Minor > M Minor > H Major > Diminished > WT > Tritone > Augmented > Maj triad > Min triad > Maj7 > Dom7 > Min7 > Min7b5 > Dim7), then Secondary Mode priority within the collection.\n")
...
if __name__ == "__main__":
    ...
    for i in range(0, 10):
        generate_filtered_modes_chart(start_column=i)
